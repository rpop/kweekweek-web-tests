...
public class KweekweekHeader {
	...
	public List addSelectedCategoryToList(String category) {
		...
		switch (category) {
			case "art-culture-item":
				text = "art-culture";
				break;
			case "entertainment-item":
				text = "entertainment";
				break;
			case "food-drink-item":
				text = "food-drink";
				break;
			case "well-being-health-item":
				text = "health-wellbeing";
				break;
			case "knowledge-item":
				text = "knowledge";
				break;
			case "networking-item":
				text = "networking";
				break;
			case "nightlife-item":
				text = "nightlife";
	...
	private WebElement signupGeneralAlert;

	public String getSignupAlertMessage() {
		Waits.waitForVisibilityOfElement(driver, signupGeneralAlert);
		return signupGeneralAlert.getText();
	}

	@FindBy(xpath = "//input[@id='user_forename']")
	private WebElement signUpFirstNameField;

	public void enterFirstNameOnSingUpField(String firstName) {
		Waits.waitForElementToBeClickable(driver, signUpFirstNameField);
	...
	public void enterLastNameOnSingUpField(String lastName) {
		//Waits.waitForElementToBeClickable(driver, signUpLastNameField);
	...
	public void enterEmailOnSingUpField(String email) {
		Waits.waitForElementToBeClickable(driver, signUpLastNameField);
...
//	}
	...
	private WebElement signUpGenderDropDownBtn;

	public void selectGenderOnSignUpForm(String gender){
		Waits.waitForElementToBeClickable(driver, signUpGenderDropDownBtn);
		signUpGenderDropDownBtn.click();
		Select oSelection = new Select(driver.findElement(By.id("select2-results-64")));
		oSelection.selectByVisibleText(gender);
	}
}